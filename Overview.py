...
def main():
    ...
    normal_text("Deep learning models are vulnerable to adversarial attacks, which deceive models into making incorrect predictions through barely noticeable alterations to input data. This project addresses this challenge by examining the Fast Gradient Sign Method (FGSM), a widely recognised and simple attack that significantly impacts model accuracy. FGSM creates adversarial examples by using the gradient of neural networks. It then multiplies a small factor, epsilon (ε), which controls the magnitude of perturbation (Goodfellow et al., 2015). At an epsilon of 8/255, this method can drastically reduce a model's accuracy, from a reliable 98.45% to a concerning 38.81%", style='text-align: justify')
